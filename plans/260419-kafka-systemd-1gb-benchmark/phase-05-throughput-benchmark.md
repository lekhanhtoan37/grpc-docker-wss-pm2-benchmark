# Phase 05: Add Throughput Measurement to Benchmark Client

**Goal:** Extend benchmark client to measure throughput (MB/s) per group in addition to latency. Report throughput delta between bridge and host groups.

**Effort:** 2-3h

---

## Task 5.1: Create throughput-aware benchmark client

**Files:**
- Create: `benchmark-client/client-throughput.js`

- [ ] **Step 1: Write client-throughput.js**
...
async function main() {
  ...
  console.log("\n=== THROUGHPUT RESULTS ===\n");
  console.log(`${"Group".padEnd(16)} ${"Msgs".padStart(10)} ${"MB/s".padStart(10)} ${"msg/s".padStart(12)} ${"vs WS".padStart(10)}`);
...
- [ ] **Step 2: Test benchmark client (with producer running)**

```bash
node benchmark-client/client-throughput.js --warmup 5 --duration 30
```

Expected: Throughput table with MB/s per group + latency percentiles

- [ ] **Step 3: Commit**

```bash
git add benchmark-client/client-throughput.js
git commit -m "feat: add throughput-aware benchmark client"
```
